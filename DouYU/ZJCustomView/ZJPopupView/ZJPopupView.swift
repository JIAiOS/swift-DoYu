...
class ZJPopupView: UIView,ZJPopupViewDelegate {
    ...
    func zj_showPopView() {
        ...
        case .ZJPopTransition?:
            self.customView.transform = self.customView.transform.concatenating(CGAffineTransform(translationX: 0, y: -(KscreenHeight-self.cusViewSize.height)/2))
            UIView.animate(withDuration: durationTime, animations: {
                self.alpha = 1.0
                self.customView.transform = self.customView.transform.concatenating(CGAffineTransform(translationX: 0, y: (KscreenHeight-self.cusViewSize.height)/2+50))
            }) { (isSuccess) in
